...
class Calculate {
    ...
    init(){
        ...
        if(document.querySelector(vm.el_btnWrap))
            document.querySelector(vm.el_btnWrap).addEventListener('click', vm.count.bind(vm))
    }
    
    // return Array
    // 顯示在最上方的公式，不包含現在正在加值的數字
    get formula(){
        if(this.calculate_objects.length === 0) 
            return ''
        else
            return this.calculate_objects
                .map(
                    item => `${item.number.join('')} ${this.format_marks(item.act)} ` 
                )
                .join('')
    }
    ...
    update_answer_element(){
        ...
        if(this.current_nums === '' && this.get_answer === null)
            ...
            el_answer.textContent = this.current_nums === '' ? this.get_answer : this.current_nums
    }
    ...
    get_new_calculate_objects(mark = ''){
        ...
        const new_numbers = current_nums[current_nums.length -1] == '.' 
    ...
    set_formula_and_clear_current_num(mark = ''){
        ...
        if(vm.get_new_calculate_objects(mark)['number'].length !== 0) 
            vm.calculate_objects = [ ...vm.calculate_objects, vm.get_new_calculate_objects(mark)]
    ...
    return_answer_string(){
        ...
        return target.reduce( (accum, currentValue, currentIndex, array) =>{
            if(currentIndex>0){
                ...
                return num(currentValue.number.join(''))
        }, 0)
    }
