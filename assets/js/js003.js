...
var currSent = document.querySelector('.currSent'); //現在剛按下去的按鈕(大字、白色的那個)， eg: a+b=c  也就是b

var numStr = ''; //基本上只會短暫的在按"="後有值，function結束就會清除

var Ans = ''; //就是藍色的那個計算式

var cal = []; //判斷如果有按過 "=" 則為ture，進入function清除計算欄後 checkEqual 變回 false
...
function count(e) {
  //判斷點擊是否為LI
  if (e.target.nodeName !== 'LI') return; //字串轉為數字
